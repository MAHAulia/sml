...
import ManifestTerimaReviewDialog from "./dialog-review";
...
export default function ManifestTerimaFormDialog({ selectedData, isOpen, setIsOpen, isView = true }: ManifestTerimaFormDialog) {
    ...
    const [reviewItem, setReviewItem] = useState<TransactionsData|null>(null)
    ...
    const moveItem = (item: TransactionsData | null) => {
        if (item == null) return
        setReviewItem(null)
        // add to selected
    ...
    return (
        <div>
            <ManifestTerimaReviewDialog 
                selectedData={reviewItem} 
                isOpen={reviewItem != null} 
                setIsOpen={() => setReviewItem(null)} 
                isView={false}
                onProcess={moveItem} />    
