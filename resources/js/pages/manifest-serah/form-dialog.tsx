...
export default function ManifestSerahFormDialog({ selectedData, isOpen, setIsOpen, isView = true }: ManifestSerahFormDialog) {
    ...
    const { data, setData, post, put, processing, errors, reset } = useForm<Required<ManifestSerahForm>>({
        to: "",
        office_to: "",
        type: "",
        action: 'add',
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogContent className={data.type === "linehaul" ? "sm:max-w-6/12" : "sm:max-w-4/12"}>
                <DialogHeader>
                    <DialogTitle>Buat Manifest Baru {isView && <Badge variant={getVariant(selectedData?.status)}>{getLabel(selectedData?.status)}</Badge>}</DialogTitle>
                    <DialogDescription>
                        Pembuatan Manifest Serah baru
                    </DialogDescription>
                </DialogHeader>
